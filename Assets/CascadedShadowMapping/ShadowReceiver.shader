//地板的材质
//这里添加了权重 和 对多个阴影贴图的处理
Shader "HCS/ShadowReceiver"
{
	SubShader
	{
		...
		Pass
		{
			...
			struct v2f
			{
				...
				float2 uv: TEXCOORD0;
				float4 shadowCoord: TEXCOORD1; //位置
				float eyeZ: TEXCOORD2; //眼睛的深度
				float4 worldPos: TEXCOORD3;
			};
			
			uniform float4x4 _gWorldToShadow; //阴影矩阵
			uniform sampler2D _gShadowMapTexture; //阴影RT
			uniform float4 _gShadowMapTexture_TexelSize; //阴影RT图尺寸
			
			uniform float4 _gLightSplitsNear; //近截面
			uniform float4 _gLightSplitsFar; //远截面
			uniform float4x4 _gWorld2Shadow[4]; //不同摄像机的投影矩阵
			
			uniform sampler2D _gShadowMapTexture0; //阴影RT0
			uniform sampler2D _gShadowMapTexture1; //阴影RT1
			uniform sampler2D _gShadowMapTexture2; //阴影RT2
			uniform sampler2D _gShadowMapTexture3; //阴影RT3
			
			uniform float _gShadowStrength; //阴影强度
